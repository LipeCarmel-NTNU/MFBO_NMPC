...
%% Construct adjacent-node log-ratio residual data
%
% For each run i, walk through the available z grid in adjacent intervals:
%
%   z_a = min(z), z_b = next grid node,
%   then z_a <- z_b and z_b <- next grid node.
%
% For each adjacent interval (z_a, z_b), define
%
%   r_iab(theta)
...
for i = 1:nRuns
    ...
    for a = 1:n-1
        ...
        % Observed log-ratio over one adjacent z-grid interval.
