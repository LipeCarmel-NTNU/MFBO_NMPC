...
disp('Steady-state at linearization...')
...
[K_opt, S_opt, e_opt] = lqrd(A, B, Q_opt, R_opt, Ts);
...
function J = lqr_tuning_objective(theta, A, B, Ts, Tf, x0, xsp, usp, model, ode_opt)
    ...
    J = 0;
    for k = 2:length(T)
        J = J + e(k,:)*Q*e(k,:)' + delta_U(k-1,:)*R*delta_U(k-1,:)';
    end
