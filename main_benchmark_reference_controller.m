...
for s = 1:numel(scenarios)
    ...
    % One row, under the same schema as the optimization runs. The run covers
    % the full horizon and uses no surrogate. phi_vintage is therefore NaN and
    % the two phi columns are 1. The row states that MATLAB measured its costs
    % and did not scale them.
    ...
    append_results_row(results_csv, 1, ts, "BENCH", out, theta, NaN);
