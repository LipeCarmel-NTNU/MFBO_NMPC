%% NMPC test runs for fidelity-1 points (full horizon, same noise as main_BO)
...
% =========================================================================
% List mode: rerun all stored controllers with theta_1 == 1
% using the same measurement-noise setup as main_BO.
% =========================================================================
cfg_run = struct();
cfg_run.mode              = "list";                 % list only
cfg_run.source_root       = "results";
cfg_run.output_root       = "results/final_fidelity_same_noise";
cfg_run.sigma_y           = [0.001 0.1 0.1]; % same measurement noise as main_BO
...
% Run a fixed list of controllers from a previous run folder with f forced to 1.0
function run_timestamp_list(cfg_run, base, run_label, timestamps)
    src_dir = fullfile(cfg_run.source_root, run_label);
    out_dir = fullfile(cfg_run.output_root, run_label + "_full_f1_same_noise");
    ensure_dir(out_dir);
...
%LOAD_FRONTIER_TIMESTAMPS Read timestamps listed in final_pareto_frontier_timestamps_only.txt.
    if ~isfile(path)
        error("Frontier timestamp file not found: %s", path);
    end
...
%SPLIT_TIMESTAMPS_BY_RUN Assign each timestamp to run1 or run2 by source file presence.
    ts_run1 = strings(0,1);
    ts_run2 = strings(0,1);
    for i = 1:numel(timestamps)
        ts = timestamps(i);
...
%WRITE_TIMESTAMP_LIST Save selected fidelity-1 timestamps for traceability.
