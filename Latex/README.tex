...
\begin{document}
	...
	\begin{align*}
		f &\in (0, 1), \\
		\theta_m &\in \mathbb{Z}_{\ge 0}, \qquad \theta_m \leq 30, \\
		\theta_p &\in \mathbb{Z}_{\ge 0}, \qquad \theta_p \leq 60, \\
		q &\in [-3,3]^3, \\
		r^{(u)} &\in [-3,3]^3, \\
		r^{(\Delta u)} &\in [-3,3]^3.
	\end{align*}
	...
	Let $y = \mathrm{Cheb3}(2\cdot f - 1; c_T)$, where \(\mathrm{Cheb3}\) is a third order Chebyshev polynomial.  
	Define:
	\[
