...
\end{align}
with Monod kinetics $\mu(S) = \mu_{\max}\, S / (K_s + S)$ and parameters $S_{\mathrm{in}} = \SI{70}{g\per L}$, $\mu_{\max} = \SI{0.273}{h^{-1}}$, $K_s = \SI{0.007}{g\per L}$, $k_d = \SI{0.006}{h^{-1}}$, $Y_{XS} = 0.42042$. State and input bounds are $V \in [0.5, 2]$, $X \in [0, 50]$, $S \in [0, 20]$, $u_i \in [0, 0.4]$, $i = 1,2,3$. Plant simulation uses \texttt{ode45} with non-negativity enforcement on $X$ and $S$.
...
Free log-weight exponents are bounded in $[-3,3]$, so the corresponding diagonal entries range from $10^{-3}$ to $10^3$.

Two BO runs are reported in this document. They differ only in which of the 12 components of $\theta$ are exposed to BO, while sharing identical bounds, acquisition settings, and noise realization.
\paragraph{Case 1 (all variables free).} Every component of $\theta$ is an optimization variable, with bounds as in Section~\ref{sec:methods}. In particular, $q_1$ is free in $[-3, 3]$ and the three input-penalty exponents $r_{u,i}$ are free in $[-3, 3]$, so $R_u$ may take any diagonal value in $[10^{-3}, 10^3]^3$.
\paragraph{Case 2 ($q_1$ fixed, $R_u$ disabled).} Two groups of variables are pinned in optimization space:
\begin{itemize}
    \item $q_1 = 0$, so the volume state-weight $Q_{11} = 10^{q_1} = 1$ is held constant;
    \item $r_{u,1} = r_{u,2} = r_{u,3} = -1000$, so the three diagonal entries of $R_u = \mathrm{diag}(10^{r_{u,1}}, 10^{r_{u,2}}, 10^{r_{u,3}})$ underflow to $0$ in double precision and the input-magnitude penalty term in the NMPC cost is effectively removed.
\end{itemize}
The remaining eight components of $\theta$ ($f, \theta_p, \theta_m, q_2, q_3, r_{\Delta u,1}, r_{\Delta u,2}, r_{\Delta u,3}$) are free with the same bounds as in Case 1.
...
\begin{itemize}
    \item \emph{Case 1} / \emph{Case 2} refer to the two independent BO tuning runs defined above. \emph{Case 1} treats all 12 components of $\theta$ (including $q_1$ and the three $r_{u,i}$ exponents) as free optimization variables in their bounds. \emph{Case 2} pins $q_1 = 0$ (fixing $Q_{11} = 1$) and pins $r_{u,1} = r_{u,2} = r_{u,3} = -1000$ in optimization space, so that $R_u = \mathrm{diag}(10^{r_{u,1}}, 10^{r_{u,2}}, 10^{r_{u,3}})$ underflows to the zero matrix and the input-magnitude penalty is effectively disabled. The remaining components of $\theta$ are free in Case 2. Wherever a figure label or numerical summary says ``Case~$i$'', it means the $i$-th BO run as defined here.
...
\caption{Evaluated samples in $(\jtv, \jtrack)$ space and Pareto-optimal points across both BO runs. Each point is one BO evaluation aggregated across the two simulation scenarios. Small black filled dots are all evaluated optimization samples from both runs pooled together. Blue filled circles mark the within-case non-dominated points of Case~1 (all 12 components of $\theta$ free, including $R_u$), and vermillion filled triangles mark the within-case non-dominated points of Case~2 ($q_1 = 0$ fixed and $R_u$ pinned to the zero matrix). Large open orange circles highlight the combined non-dominated frontier obtained when the two runs are merged into a single pool.}
\label{fig:pareto}
\end{figure}
...
A second observation is that the two runs specialize on different segments of the front. Case~1 populates the low-$\jtv$ tail ($\jtv$ down to $0.026$ but $\jtrack$ up to $22{,}781$), while Case~2 populates the low-$\jtrack$ region ($\jtrack$ as low as $12{,}221$ with $\jtv \in [0.13, 0.39]$). Both runs concentrate at $\Nc = 1$, but Case~1's Pareto subset frequently also picks $\Np = 1$ (5 of 9 candidates), whereas Case~2 picks $\Np = 1$ in only 4 of 10. When $\Nc = \Np = 1$, the NMPC optimization horizon collapses to one stage cost plus the terminal $P$, so the resulting policy reduces to the LQR feedback law that built $P$ applied to the nonlinear model at the current state: a smooth, conservative controller (low $\jtv$) that converges slowly (higher $\jtrack$). When $\Nc = 1$ but $\Np > 1$, a single control move is committed but evaluated against an $\Np$-step prediction of free-state evolution under that move, closed off by the same terminal $P$. This makes the controller more anticipatory, lowering $\jtrack$ at the cost of larger move-to-move corrections.

The specialization is consistent with the design difference between the two cases. In Case~1 the input-magnitude penalty $R_u$ is exposed to BO: inspection of the low-$\jtv$ Pareto subset shows that BO drives $r_{u,1}$ and $r_{u,2}$ to the upper bound $r_{u,i} = 3$ (i.e.\ $R_{u,ii} = 10^3$) and pairs them with a large $R_{\Delta u}$, so the controller carries two compounded smoothing mechanisms and tends to collapse to the LQR-like $\Np = 1$ sub-mode. In Case~2, $R_u$ is pinned to zero by construction and the only remaining input-shaping mechanism is $R_{\Delta u}$. With this single, weaker handle on input smoothness, BO instead exploits the prediction horizon: longer $\Np$ enables anticipatory state shaping under a single committed move, which improves tracking ($\jtrack$ as low as $12{,}221$) while keeping $\jtv$ moderate ($0.13$--$0.39$). The two Pareto specializations therefore reflect two genuinely different controller architectures admitted by the two configurations (one with two smoothing knobs and a short horizon, one with a single smoothing knob and a longer horizon), rather than two finite-budget realisations of the same architecture.
...
\caption{Per-evaluation runtime over iteration. Left subpanel: Case~1 (BO run with all 12 components of $\theta$ free). Right subpanel: Case~2 (BO run with $q_1 = 0$ and $R_u = 0$ pinned in optimization space). The shaded region in each subpanel highlights the 20 DOE iterations preceding the optimization phase.}
\label{fig:runtime_iter}
\end{subfigure}
\hfill
\begin{subfigure}{0.49\linewidth}
\includegraphics[width=\linewidth]{runtime_cumulative_run1_run2.pdf}
\caption{Cumulative wall-clock runtime. The blue curve is Case~1 (all 12 components of $\theta$ free) and the vermillion curve is Case~2 ($q_1 = 0$ and $R_u$ pinned to zero).}
...
The second concerns the structure of the Pareto front. Case~1 (all 12 components of $\theta$ free) and Case~2 ($q_1 = 0$ and $R_u$ pinned to zero) specialize on opposite tails of the frontier in a way that is mechanistically tied to which smoothing mechanisms each run has at its disposal. Both runs collapse to $\Nc = 1$ on the Pareto set. Case~1 has access to both $R_u$ and $R_{\Delta u}$ as free variables, and BO drives $r_{u,1}$ and $r_{u,2}$ to the upper bound while choosing a large $R_{\Delta u}$; this doubly-smoothed configuration frequently also picks $\Np = 1$, reducing the controller to the LQR feedback law derived from~\eqref{eq:Plyap} applied to the nonlinear plant at the current state, which is naturally smooth (low $\jtv$) but slow (higher $\jtrack$). Case~2 has its input-magnitude penalty disabled by construction; with only $R_{\Delta u}$ as a smoothing knob, BO compensates by enlarging the prediction horizon, so $\Np$ tends to exceed $1$ and the resulting controllers are anticipatory through the prediction window, giving sharper tracking but larger move-to-move variation. The two runs are not contradictory; they are the two controller architectures admitted by their respective parameter configurations, and the BO finite budget unfolded into each architecture's natural Pareto tail. The benchmark sits strictly inside the Pareto set, achieving comparable $\sse$ to the best Case~2 controllers but with SSdU $8$ to $22\times$ larger --- it pays a large control-variation cost for no tracking advantage on this plant.
...
The two BO runs specialize on opposite tails of the Pareto front in a way that is mechanistically explained by the smoothing mechanisms each run has at its disposal and the resulting choice of $\Np$. Case~1 (all variables free) drives $R_u$ and $R_{\Delta u}$ high and collapses to the LQR-like $\Nc = \Np = 1$ regime, giving the low-$\jtv$ tail. Case~2 ($R_u$ pinned to zero, $q_1$ fixed) cannot use the input-magnitude penalty and instead lengthens the prediction horizon, giving the low-$\jtrack$, $\Nc = 1, \Np > 1$ anticipatory regime. The benchmark sits strictly inside this front. The setpoint-schedule analysis demonstrates that the BO-Pareto-then-engineer workflow transfers to scenarios outside the optimization regime. State-wise settling is the principal gap: minimizing $(\jtrack, \jtv)$ does not guarantee balanced settling across outputs, particularly for substrate, and a tighter settling-aware objective formulation is the recommended next iteration.
